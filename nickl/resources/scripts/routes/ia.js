jQuery(document).ready(function ($) {
  ...
  // 0. Init Voice Input (Web Speech API)
  initVoiceInput();

  // Style Loader Overlay
  $('<style>')
    .prop('type', 'text/css')
    .html('#ia-loader-overlay { display: none; position: fixed; top: 0; left: 0; width: 100%; height: 100%; background: rgba(255,255,255,0.9); z-index: 999999; flex-direction: column; justify-content: center; align-items: center; } .ia-spinner-lg { width: 50px; height: 50px; border: 5px solid #f3f3f3; border-top: 5px solid #2271b1; border-radius: 50%; animation: ia-spin 1s linear infinite; } @keyframes ia-spin { 0% { transform: rotate(0deg); } 100% { transform: rotate(360deg); } }')
    .appendTo('head');

  // Inject Overlay HTML
  $('body').append('<div id="ia-loader-overlay"><div class="ia-spinner-lg"></div><div id="ia-loader-text" style="margin-top:20px; font-size:18px; font-weight:600; color:#2271b1;">Veuillez patienter...</div></div>');

  function showLoader(text) {
    $('#ia-loader-text').text(text);
    $('#ia-loader-overlay').css('display', 'flex');
  }

  function hideLoader() {
    $('#ia-loader-overlay').hide();
  }
  ...
  $(document).on('click', '#generateaicontent', function (e) {
    ...
    showLoader('Interrogation de l\'IA...');

    // 2. Appel AJAX vers le serveur (PHP -> Claude)
    $.ajax({
      url: AjaxIa.ajax_url,
      type: 'POST',
      dataType: 'json',
      data: {
        action: 'generer_contenu_ia_claude',
        prompt: promptText,
      ...
      success: function (response) {
        if (response.success) {
          ...
          showLoader('Construction des modules...');
 ...
 */
  function buildAcfLayouts(modulesData, $btn, originalText) {
    ...
    if (!flexibleSchemas) {
        ...
        console.warn('flexibleSchemas manquant via acf.getField dans ia.js, tentative fallback jQuery...');
                     ...
                     var processNextColumn = function(index) {
                         ...
                         var pollForField = function() {
                            ...
                            if (!res.$row && attempt === 0) {
                                ...
                                // Don't inc attempt here, give it time to appear
      ...
      modulesList.forEach(function (subMod) {
        ...
        try {
          ...
          if (res) {
              ...
              var dataToPass = subMod.data || subMod;
              if (dataToPass) {
                  handler(dataToPass, $subRow);
              } else {
                  console.warn('[processSubModules] subMod data is empty for', layoutName);
              }
          } else {
    ...
    function fillCommonFields(data, $row) {
      if (!data) {
          console.warn('[fillCommonFields] Data is missing/undefined so we cannot fill common fields.');
          return;
      }
  ...
  function initVoiceInput() {
    var $field = $('.acf-field[data-name="content_html"]');
    if (!$field.length) return;
    ...
    // Avoid double init
    if ($label.find('#btn-voice-ia').length) return;

    var $btn = $('<a href="#" id="btn-voice-ia" class="button button-secondary" style="margin-left: 10px; vertical-align: middle;"><span class="dashicons dashicons-microphone" style="vertical-align: text-top;"></span> Dictée</a>');
    $label.append($btn);
    ...
    recognition.onresult = function (event) {
      // Loop through results to handle continuous segments
      for (var i = event.resultIndex; i < event.results.length; ++i) {
        if (event.results[i].isFinal) {
          var transcript = event.results[i][0].transcript.trim();
          var current = $textarea.val();
          // Add space if needed
          $textarea.val(current + (current && !current.endsWith(' ') ? ' ' : '') + transcript).trigger('change');
        }
      }
    };

    // Fired when service disconnects or stop is called
    recognition.onend = function () {
      isRecording = false;
      $btn.removeClass('active').css('color', '');
      $btn.find('span').removeClass('dashicons-controls-rec').addClass('dashicons-microphone');
      $btn.html('<span class="dashicons dashicons-microphone" style="vertical-align: text-top;"></span> Dictée');
    };

    recognition.onerror = function (event) {
      if (event.error === 'no-speech') {
        console.warn('Voice info: No speech detected (timeout).');
      } else {
    ...
    $btn.on('click', function (e) {
      ...
      if (isRecording) {
        ...
        recognition.start();
        isRecording = true;
        $(this).addClass('active').css('color', '#d63638');
        $(this).html('<span class="dashicons dashicons-controls-rec" style="vertical-align: text-top;"></span> Arrêter Dictée');
      }
    });
  }
