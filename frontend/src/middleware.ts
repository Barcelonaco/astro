...
export const onRequest = defineMiddleware(async (context, next) => {
  ...
  // Serve /uploads/media/_optimized/ — try disk cache first, proxy to backend for processing if not cached
  if (pathname.startsWith('/uploads/media/_optimized/')) {
    ...
    // Serve from disk cache if available (avoids HTTP proxy)
    const resp = serveFile(cachePath);
    if (resp) return resp;

    // Not cached — proxy to backend for processing
    try {
