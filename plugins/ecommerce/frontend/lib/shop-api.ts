...
export async function getCart(billingCountry?: string): Promise<Cart> {
  const qs = billingCountry ? `?billing_country=${encodeURIComponent(billingCountry)}` : ''
  const c = await shopRequest<Cart>('GET', `/cart${qs}`)
  return c || emptyCart()
}
